...
import { buildChartOverlays, type OhlcvBar } from "@/lib/technicalScanner";
...
export function TechnicalChartPanel({ symbol, bars, sourceLabel, sourceUrl, observedAt, lastSuccessfulAt, delayMinutes, errorMessage }: TechnicalChartPanelProps) {
  const visibleBars = bars.slice(-80);
  const overlays = useMemo(() => buildChartOverlays(visibleBars), [visibleBars]);
  ...
  const overlayPoints = (key: "sma10" | "sma20" | "sma50") => overlays
    .map((point, index) => point[key] === null ? null : `${index * step + step / 2},${priceY(point[key])}`)
  ...
  return <article className="overflow-hidden rounded-2xl border border-white/12 bg-[#101411] p-5 sm:p-6">
    ...
    {hasBars ? <div className="mt-5 overflow-hidden rounded-xl border border-white/10 bg-[#0b0e0c] p-2">
      <svg viewBox={`0 0 ${chartWidth} ${chartHeight}`} role="img" aria-label={`${symbol} mum ve hacim grafiği`} className="h-auto w-full">
        ...
        {overlayPoints("sma10") && <polyline points={overlayPoints("sma10")} fill="none" stroke="#8ab5e3" strokeWidth="1.6" strokeLinejoin="round" strokeLinecap="round" opacity=".95" />}
        {overlayPoints("sma20") && <polyline points={overlayPoints("sma20")} fill="none" stroke="#d9c27d" strokeWidth="1.6" strokeLinejoin="round" strokeLinecap="round" opacity=".95" />}
        {overlayPoints("sma50") && <polyline points={overlayPoints("sma50")} fill="none" stroke="#e7a2b7" strokeWidth="1.6" strokeLinejoin="round" strokeLinecap="round" opacity=".95" />}
      </svg>
      <div className="flex flex-wrap gap-x-4 gap-y-2 px-2 pb-1 pt-3 text-[10px] text-[#9ca79c]"><span className="inline-flex items-center gap-1.5"><i className="h-1.5 w-4 rounded bg-[#8ab5e3]"/>SMA(10)</span><span className="inline-flex items-center gap-1.5"><i className="h-1.5 w-4 rounded bg-[#d9c27d]"/>SMA(20)</span><span className="inline-flex items-center gap-1.5"><i className="h-1.5 w-4 rounded bg-[#e7a2b7]"/>SMA(50)</span><span className="text-[#718071]">Katmanlar yalnızca gösterilen mumlardan hesaplanır.</span></div>
    ...
    <p className="mt-2 mono text-[9px] text-[#748074]">SON BAŞARILI GÜNCELLEME · {formatObservedAt(lastSuccessfulAt)}</p>
